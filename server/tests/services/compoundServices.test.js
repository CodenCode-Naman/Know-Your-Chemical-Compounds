...
describe('Compounds Services', () => {
  ...
  describe('upsertCompounds', () => {
    it('should upsert compounds and return the upserted compounds', async () => {
      const mockCompoundsArray = [mockCompounds.compounds[0]];
      jest.spyOn(db.Compound, 'bulkCreate').mockResolvedValue(mockCompoundsArray);
      const result = await compoundsServices.upsertCompounds(mockCompoundsArray);
      expect(db.Compound.bulkCreate).toHaveBeenCalled();
      expect(result).toEqual(mockCompoundsArray);
    });

    it('should throw an error if upsertCompounds fails', async () => {
      jest.spyOn(db.Compound, 'bulkCreate').mockRejectedValue(new Error('Error'));
      await expect(compoundsServices.upsertCompounds([])).rejects.toThrow('Error');
    });
  });
});