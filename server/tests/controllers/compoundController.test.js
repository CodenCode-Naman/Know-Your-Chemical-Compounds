...
const mockCompounds = require('../mockData/chemicalCompounds.json');
...
describe('Compounds Controller', () => {
  describe('getAllCompounds', () => {
    ...
    it('should return a message if no compounds are found', async () => {
      ...
      expect(res.json).toHaveBeenCalledWith({ message: 'Compounds does not exists' });
    });
  ...
  describe('getCompoundById', () => {
    ...
    it('should return a message if no compound is found', async () => {
      ...
      expect(compoundsServices.getCompoundById).toHaveBeenCalled();
      expect(res.status).toHaveBeenCalledWith(404);
      expect(res.json).toHaveBeenCalledWith({ message: 'Compound with id not exist' });
    });
  ...
  describe('updateCompoundById', () => {
    ...
    it('should return a message if no compound is found', async () => {
      ...
      expect(compoundsServices.updateCompound).toHaveBeenCalled();
      expect(res.status).toHaveBeenCalledWith(404);
      expect(res.json).toHaveBeenCalledWith({ message: 'Compound does not exists' });
    });
  ...
  describe('deleteCompoundById', () => {
    ...
    it('should return a message if no compound is found', async () => {
      ...
      expect(compoundsServices.deleteCompound).toHaveBeenCalled();
      expect(res.status).toHaveBeenCalledWith(404);
      expect(res.json).toHaveBeenCalledWith({ message: 'Compound does not exist' });
    });
  ...
  describe('upsertCompounds', () => {
    it('should upsert compounds and return the upserted compounds', async () => {
      const req = { body: [mockCompounds.compounds[0]] };
      const res = {
        status: jest.fn().mockReturnThis(),
        json: jest.fn(),
      };
      jest.spyOn(compoundsServices, 'upsertCompounds').mockResolvedValue(req.body);
      await compoundsController.upsertCompounds(req, res);
      expect(compoundsServices.upsertCompounds).toHaveBeenCalled();
      expect(res.status).toHaveBeenCalledWith(200);
      expect(res.json).toHaveBeenCalledWith(req.body);
    });

    it('should throw an error if upsertCompounds fails', async () => {
      const req = { body: [mockCompounds.compounds[0]] };
      const res = {
        status: jest.fn().mockReturnThis(),
        json: jest.fn(),
      };
      jest.spyOn(compoundsServices, 'upsertCompounds').mockRejectedValue(new Error('Error'));
      await compoundsController.upsertCompounds(req, res);
      expect(compoundsServices.upsertCompounds).toHaveBeenCalled();
      expect(res.status).toHaveBeenCalledWith(500);
      expect(res.json).toHaveBeenCalledWith({ message: 'Error' });
